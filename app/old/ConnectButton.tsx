...
const ConnectButton = () => {
  ...
  useEffect(() => {
    ...
    const ready = wallet.adapter.readyState; // Installed / Loadable / NotDetected
    const isPhantom = name === 'Phantom';
    ...
    // 🔥 ПЕРЕХВАТ ТОЛЬКО В ПРАВИЛЬНЫЙ МОМЕНТ
    if (
      isPhantom &&
      isAndroid &&
      !isInPhantom &&
      !connected &&
      ready === 'Installed' &&          // 👈 ВАЖНО
      ...
      // ✅ ПРАВИЛЬНЫЙ PHANTOM DEEPLINK
      const phantomLink = `https://phantom.app/ul/browse/${url}?ref=${encodeURIComponent(window.location.origin)}`;

      console.log('🔥 OPENING PHANTOM APP (ANDROID FIX)');

      // пробуем открыть Phantom
      window.location.href = phantomLink;

      // ⏳ fallback ТОЛЬКО ЕСЛИ НЕ ОТКРЫЛОСЬ
      const start = Date.now();

      setTimeout(() => {
        // если через 2 секунды страница всё ещё активна → app не открылся
        if (Date.now() - start < 2500) {
          console.log('⚠️ Phantom not opened, redirecting to Play Market');
